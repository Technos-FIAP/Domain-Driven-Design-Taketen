...
private TipoExercicioBO tipoExercicioBO = new TipoExercicioBO();	
	...
	public Response cadastroRs (TipoExercicio tipoExercicio, @Context UriInfo uriInfo ) throws ClassNotFoundException, SQLException {
		...
		builder.path(Integer.toString(tipoExercicio.getIdTipoExercicio()));
	...
	@Path("/{id}")
	@Consumes(MediaType.APPLICATION_JSON)
	public Response atualizaRs (TipoExercicio tipoExercicio, @PathParam("id") int id) throws SQLException, ClassNotFoundException {
		tipoExercicioBO.atualizarBo(tipoExercicio);
